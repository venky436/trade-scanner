  ...
  return (
    <div className="flex items-center gap-1">
      {Array.from({ length: cells }).map((_, i) => (
        <span
          key={i}
          className={`h-2 w-3 rounded-sm ${i < filled ? directionFill(dir) : "bg-zinc-200 dark:bg-zinc-800"}`}
        />
      ))}
    </div>
  );
      ...
      <div className={`relative overflow-hidden rounded-2xl border ${ZONE_BORDER[zone]} bg-gradient-to-br ${ZONE_GRADIENT[zone]} bg-white dark:bg-zinc-950/60 p-7`}>
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            <div
              className="text-4xl font-bold tabular-nums text-zinc-900 dark:text-zinc-50"
