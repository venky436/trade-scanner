...
// can compare momentum/pressure/volatility at a glance.
  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
          className={`h-1.5 w-2.5 rounded-sm ${i <= filled ? directionFill(dir) : "bg-zinc-200 dark:bg-zinc-800"}`}
        />
      ))}
    </div>
  );
...
// Subtle price-flash on tick — emerald if price rose vs last render, rose if it
// fell. Drops back to normal after ~600ms. Communicates "this is live" without
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        ...
        <div className={`shrink-0 rounded-xl px-2 py-1 transition-shadow duration-300 ${flashRing}`}>
          <div
            className="text-right text-2xl font-bold tabular-nums text-zinc-900 dark:text-zinc-50"
      ...
      {/* Live freshness footer */}
      <div className="mt-4 flex items-center gap-2">
        <span className="size-1.5 animate-pulse rounded-full bg-emerald-500 dark:bg-emerald-400/80" />
