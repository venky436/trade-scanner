...
|---|---|---|
| BREAKOUT | Price ABOVE resistance + 0.2% buffer + STRONG_BUY (or BUY with conf>=0.6) + UP/STRONG_UP momentum + quality > 0.5 + conf >= 0.5 | BUY (confirmed) |
| BREAKDOWN | Price BELOW support - 0.2% buffer + STRONG_SELL + STRONG_DOWN + \|quality\| > 0.5 | SELL (confirmed) |
...
13. **Market filter layer** — Blocks signals when market/stock is too quiet (DEAD < 0.10%, SLOW < 0.30%, per-stock < 0.2%). See [`docs/market-filter.md`](./market-filter.md)
14. **Accuracy requires CONFIRMED stage** — Only signals with `signal.type` (BREAKOUT/BOUNCE/REJECTION/BREAKDOWN) from CONFIRMED stage get tracked. MOMENTUM/PRESSURE stages bypass S/R confirmation and are excluded.
